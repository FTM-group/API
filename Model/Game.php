            ...
            try{
                ...
                try{
                    ...
                    try{
                        ...
                        include '../Bdd/deconnexion.php';
                        return array('status'=>'error', 'error' => 'nb_max_players');
                    }
                }
                catch (Exception $e){
                    $error = $e->getCode();
                    $errorMessage = $e->getMessage();
                    include '../Bdd/deconnexion.php';
                    return array('status'=>'error', 'error' => 'genres');
                }
            }
            catch (Exception $e){
                $error = $e->getCode();
                ...
                if ($error == "23000"){
                    if (strpos($errorMessage, 'name_game')) {
                        return array('status'=>'error', 'error' => 'name_game');
                    }
                    else if (strpos($errorMessage, 'headline_game')) {
                        return array('status'=>'error', 'error' => 'headline_game');
                    }
        ...
        try{
            ...
            try{
                ...
                try{
                    ...
                    try{
                        ...
                        $error = $e->getCode();
                        $errorMessage = $e->getMessage();
                        return array('status'=>'error', 'error' => 'add_genres');
                    }
                }
                catch(Exception $e){
                    $error = $e->getCode();
                    $errorMessage = $e->getMessage();
                    return array('status'=>'error', 'error' => 'delete_genres');
                }
            }
            catch(Exception $e){
                $error = $e->getCode();
            ...
            if ($error == "23000"){
                if (strpos($errorMessage, 'name_game')) {
                    ...
                    return array('status'=>'error', 'error' => 'on_off_game');
                }
